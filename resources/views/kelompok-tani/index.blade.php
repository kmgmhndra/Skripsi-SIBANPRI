                ...
                @auth
                    @if(auth()->user()->role === 'admin')
                        <button onclick="showModalKonfirmasi()"
                            class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600" {{ count($kelompokTani) == 0 ? 'disabled' : '' }}>
                            Mulai Seleksi
                        </button>
                    @endif
                @endauth
        ...
        function filterTableByKecamatan(kecamatanId) {
            ...
            rows.forEach(row => {
                if (kecamatanId === 'all') {
                    ...
                    row.style.display = String(row.getAttribute("data-kecamatan")) === String(kecamatanId) ? "table-row" : "none";
                }
            });
        }
