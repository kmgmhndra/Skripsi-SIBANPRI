...
@section('content')
<div class="max-w-7xl sm:w-auto mx-auto p-6 bg-white shadow-lg rounded-lg">
    <h1 class="text-3xl font-bold text-gray-800 mb-4 text-center md:text-left">Daftar Kriteria</h1>

    <!-- Form Tambah Kriteria -->
    @auth
    @if(auth()->user()->role === 'admin')

    <form id="formTambah" class="flex flex-col gap-2 mb-6 md:flex-row">
        <input type="text" id="namaKriteria" name="nama" placeholder="Nama Kriteria"
            class="border p-2 rounded w-full md:flex-1" required>
        <input type="number" id="urutanKriteria" name="urutan" placeholder="Urutan"
            class="border p-2 rounded w-full md:w-20" min="1" required>
        <select id="jenisKriteria" name="jenis" class="border p-2 rounded w-full md:w-auto">
            <option value="benefit">Benefit</option>
            <option value="cost">Cost</option>
        </select>
        <button type="submit"
            class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 w-full md:w-auto">Simpan</button>
    </form>
    @endif
    @endauth
