...
<div class="max-w-7xl mx-auto p-6 bg-white shadow-lg rounded-lg">
    ...
    <form id="formTambah" class="flex gap-2 mb-6">
        <input type="text" id="namaKriteria" name="nama" placeholder="Nama Kriteria" class="border p-2 flex-1 rounded" required>
        <input type="number" id="urutanKriteria" name="urutan" placeholder="Urutan" class="border p-2 w-20 rounded" min="1" required>
    ...
    <table class="w-full border-collapse border border-gray-300">
        ...
        <tbody id="kriteriaTbody">
            ...
            <tr data-id="{{ $k->id }}">
                ...
                <td class="border p-2">
                    <button onclick="editKriteria({{ $k->id }}, '{{ $k->nama }}', '{{ $k->jenis }}', {{ $k->urutan }})" class="bg-yellow-500 text-white px-3 py-1 rounded">Edit</button>
                    <button onclick="hapusKriteria({{ $k->id }}, '{{ $k->nama }}')" class="bg-red-500 text-white px-3 py-1 rounded">Hapus</button>
                </td>
            </tr>
            @endforeach
        </tbody>
